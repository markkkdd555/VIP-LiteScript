...
echo "# //	email: ilyassnajhi10@gmail.com"
echo "# //      telegram: https://t.me/IlyassExE"
echo "# //===================================================="
sleep 3
...
function LOGO() {
 ...
 ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───
 ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───
    │    ${YELLOW}Copyright${FONT} (C)$GRAY https://github.com/FasterExE$NC     │
    └───────────────────────────────────────────────┘
...
MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/markkkdd555/VIP-LiteScript/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
        TIMEDATE
    else
...
KYTPROJECT() {
    curl -sS https://raw.githubusercontent.com/markkkdd555/VIP-LiteScript/main/ip >/root/tmp
...
apete_eee() {
    ...
    if [ -f /home/needupdate ]; then
        ...
        green "Contact telegram https://t.me/IlyassExE"
        sleep 8
        exit 0
    fi
}
...
clear
LOGO
echo -e "${RED}DO NOT INSTALL THIS SCRIPT USING A VPN CONNECTION!!!${FONT}"
echo -e ""
echo -e "${Green}DNS POINTING${FONT}(DNS-resolved IP address of the domain)"
echo -e "${Green}Go To The Installation Process In 8 Seconds More !!!"
echo ""
sleep 8
### System Information
TANGGAL=$(date '+%Y-%m-%d')
...
function base_package() {
    ...
    gem install lolcat
    print_ok "Successfully installed the required packages"
}
clear
...
Documentation=https://github.com/IlyassExE
...
function download_config(){
    print_install "Installs the configuration package configuration"
...
# ~/.profile: executed by Ilyass login shells.
if [ "$BASH" ]; then
    if [ -f ~/.bashrc ]; then
        . ~/.bashrc
    fi
...
print_ok "Complete installation of additional modules"
}
...
function finish(){
    USRSC=$(curl -sS https://raw.githubusercontent.com/FasterExE/VIP-LiteScript/ipuk/ip | grep $MYIP | awk '{print $2}')
    EXPSC=$(curl -sS https://raw.githubusercontent.com/FasterExE/VIP-LiteScript/ipuk/ip | grep $MYIP | awk '{print $3}')
